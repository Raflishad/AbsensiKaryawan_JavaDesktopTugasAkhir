 ...
 */
public class adminJabatan extends javax.swing.JFrame {
         Connection conn;
    ...
    public Object getCellEditorValue() {
     ...
     private String generateIDJabatan() {
    String newID = "J001"; // ID awal jika database kosong

    try {
        String sql = "SELECT ID_JABATAN FROM JABATAN ORDER BY ID_JABATAN DESC LIMIT 1";
        Statement stmt = conn.createStatement();
        ResultSet rs = stmt.executeQuery(sql);

        if (rs.next()) {
            String lastID = rs.getString("ID_JABATAN"); // Ambil ID terakhir
            int number = Integer.parseInt(lastID.substring(1)) + 1; // Ambil angka lalu tambah 1
            newID = String.format("J%03d", number); // Format ulang dengan B + angka 3 digit
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
    ...
    String idJabatan = generateIDJabatan(); // Ambil ID otomatis
    String namaJabatan = jTextField1.getText().trim();

    if (namaJabatan.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Nama Jabatan tidak boleh kosong!", "Peringatan", JOptionPane.WARNING_MESSAGE);
        return;
    }

    try {
        String sql = "INSERT INTO JABATAN (ID_JABATAN, NAMA_JABATAN) VALUES (?, ?)";
        PreparedStatement stmt = conn.prepareStatement(sql);
        stmt.setString(1, idJabatan);
        stmt.setString(2, namaJabatan);
        stmt.executeUpdate();
    ...
    String nama = JOptionPane.showInputDialog("Edit Nama Jabatan", model.getValueAt(row, 1).toString());
    if (nama != null) {
        try {
            PreparedStatement stmt = conn.prepareStatement("UPDATE JABATAN SET NAMA_JABATAN = ? WHERE ID_JABATAN = ?");
...
private void deleteData(int row) {
    ...
    if (confirm == JOptionPane.YES_OPTION) {
        try {
            PreparedStatement stmt = conn.prepareStatement("DELETE FROM JABATAN WHERE ID_JABATAN = ?");
            stmt.setString(1, id);
            stmt.executeUpdate();
            loadData(); // Refresh tabel
        } catch (SQLException e) {
    ...
    private void initComponents() {
        ...
        jButton1.setText("Tambah Jabatan");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(adminJabatan.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new adminJabatan().setVisible(true);
            }
        });
    }
